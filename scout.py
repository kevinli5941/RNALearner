...
parser.add_argument("--patience", type=int, default=10, help='Number of consecutive epochs with increasing validation loss required for early stopping.')
...
label_dict, label = np.unique(np.array(data.obs['kmeans30']), return_inverse=True)  # Convert strings categorical to integrate categorical, and label_dict[label] can be restored
...
train_accs = []
val_accs = []
save_simple_ckpt(model, model_name + "_begin", ckpt_dir)
